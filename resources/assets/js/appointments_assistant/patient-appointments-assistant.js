document.addEventListener('turbo:load',
    loadPatientPanelAppointmentAssistantFilterData)

function loadPatientPanelAppointmentAssistantFilterData () {
...
listenClick('.patient-panel-apptment-delete-btn', function (event) {
    ...
    let patientPanelApptmentRecordUrl = !isEmpty(userRole) ? route(
        'patients.assistants-appointment.destroy',
        patientPanelApptmentRecordId) : route('assistants-appointment.destroy',
...
//     cancelAppointment(route('patients.cancel-status'), Lang.get('js.appointment'),
//         appointmentId)
// })

listenClick('.patient-cancel-assistant-appointment', function (event) {
    let appointmentId = $(event.currentTarget).attr('data-id');
    cancelAppointment(route('patients.cancel-assistant-status'), Lang.get('js.appointment'), appointmentId);
});
...
function deleteItemAjax (url, header, appointmentId) {
    ...
    $.ajax({
        url: route('patients.cancel-assistant-status'),
        type: 'POST',
