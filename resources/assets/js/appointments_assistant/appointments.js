document.addEventListener('turbo:load', loadAppointmentAssistantFilterDate)

let appointmentFilterDate = $('#appointmentAssistantDateFilter')

function loadAppointmentAssistantFilterDate () {
    if (!$('#appointmentAssistantDateFilter').length) {
        return
    }
    ...
    function cb (start, end) {
        $('#appointmentAssistantDateFilter').val(
            start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'))
    }

    $('#appointmentAssistantDateFilter').daterangepicker({
...
listenClick('#appointmentResetFilter', function () {
    ...
    $('#appointmentAssistantDateFilter').data('daterangepicker').setStartDate(moment().startOf('week').format('MM/DD/YYYY'))
    $('#appointmentAssistantDateFilter').data('daterangepicker').setEndDate(moment().endOf('week').format('MM/DD/YYYY'))
    hideDropdownManually($('#apptmentFilterBtn'), $('.dropdown-menu'));
})

listenClick('#assistantApptResetFilter', function () {
    $('#assistantApptPaymentStatus').val(1).trigger('change')
    $('#appointmentAssistantDateFilter').data('daterangepicker').setStartDate(moment().startOf('week').format('MM/DD/YYYY'))
    $('#appointmentAssistantDateFilter').data('daterangepicker').setEndDate(moment().endOf('week').format('MM/DD/YYYY'))
    hideDropdownManually($('#assistantAptFilterBtn'), $('.dropdown-menu'));
})
...
listenChange('#appointmentAssistantDateFilter', function () {
...
listenChange('#paymentStatus', function () {
    window.livewire.emit('changeDateFilter', $('#appointmentAssistantDateFilter').val())
    window.livewire.emit('changeStatusFilter', $('#appointmentStatus').val())
    window.livewire.emit('changePaymentTypeFilter', $(this).val())
})

listenChange('#assistantApptPaymentStatus', function () {
    window.livewire.emit('changeDateFilter', $('#appointmentAssistantDateFilter').val())
