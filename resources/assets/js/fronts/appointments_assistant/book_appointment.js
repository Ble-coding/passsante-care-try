...
function loadFrontAppointmentData() {
    ...
    if (!($("#appointmentAssistantId").val() == "")) {
        $(dateEle).removeAttr("disabled");
        $.ajax({
            url: route("get-service"),
            type: "GET",
            data: {
                appointmentAssistantId: $("#appointmentAssistantId").val(),
            },
    ...
    $.ajax({
        ...
        type: "GET",
        data: {
            adminAppointmentAssistantId: $("#appointmentAssistantId").val(),
...
listenChange("#appointmentAssistantId", function (e) {
    e.preventDefault();
...
listenSubmit("#frontAppointmentBook", function (e) {
    ...
    let assistant = $("#appointmentAssistantId").val().trim();
    ...
    if (assistant == "") {
        response =
            '<div class="gen alert alert-danger">' +
            Lang.get("js.assistant_required") +
            "</div>";
        $(".book-appointment-message").html(response).delay(5000).hide("slow");
        $(window).scrollTop($(".appointment-form").offset().top);
        return false;
        ...
        success: function (result) {
            if (result.success) {
                ...
                if (result.data.payment_type == manually) {
                    Turbo.visit(
                        route("manually-assistant-payment", {
