document.addEventListener('turbo:load', loadAssistantAppointmentCalendar)

let popover
let popoverState = false
let appointmentStatusId = null
let assistantAppointmentCalendar
let data = {
    id: '',
    uId: '',
    eventName: '',
...
function loadAssistantAppointmentCalendar () {
    initCalendarApp()
    init()
}

const initCalendarApp = function () {
    if (usersRole != 'assistant') {
        return;
    }
    let calendarEl = document.getElementById('assistantAppointmentCalendar')

    if (!$(calendarEl).length) {
        return
    }
    let lang = $('.currentLanguage').val()
    assistantAppointmentCalendar = new FullCalendar.Calendar(calendarEl, {
        locale: lang,
        ...
        events: function (info, successCallback, failureCallback) {
            $.ajax({
                url: route('assistants.appointments-assistant.calendar'),
                type: 'GET',
                data: info,
    ...
    assistantAppointmentCalendar.render()
}

const init = () => {
    if (!$('#assistantAppointmentCalendarModal').length) {
        return
    }
    const viewElement = document.getElementById(
        'assistantAppointmentCalendarModal')
...
listenChange('.assistant-apptnt-calendar-status-change', function () {
    if (!$(this).val()) {
        return false
    }
    ...
    $.ajax({
        url: route('assistants.change-status-assistant', appointmentId),
        type: 'POST',
        data: {
        ...
        success: function (result) {
            ...
            $('#assistantAppointmentCalendarModal').modal('hide')
            assistantAppointmentCalendar.refetchEvents()
        },
    })
})
