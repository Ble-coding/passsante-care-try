document.addEventListener('turbo:load', loadAssistantHoliday)
document.addEventListener('turbo:load', loadHoliday)

function loadAssistantHoliday () {

    let lang = $('.currentLanguage').val()
    $('#assistantHolidayDate').flatpickr({
        'locale': lang,
        minDate: new Date().fp_incr(1),
        disableMobile: true,
    })

    listenClick('.assistant-holiday-delete-btn', function (event) {
    ...
    if (!$('#assistantHolidayDateFilter').length) {
        return
    }
    let startDate = moment().startOf('week')
    let endDate = moment().endOf('week')
    function cb (start, end) {
        $('#assistantHolidayDateFilter').val(
            start.format('MM/DD/YYYY') + ' - ' + end.format('MM/DD/YYYY'))
    }
    let datePicker = $('#assistantHolidayDateFilter').daterangepicker({
        startDate: startDate,
        endDate: endDate,
        opens: 'left',
        showDropdowns: true,
...
listenChange('#assistantHolidayStatus', function () {
    $('#assistantHolidayStatus').val($(this).val())
...
function loadHoliday () {
   ...
   let holidayPcker = $('#holidayDateFilter').daterangepicker({
        ...
        // ranges: {
        //     [Lang.get('messages.datepickerss.today')]: [moment(), moment()],
...
listenChange('#holidayDateFilter,#assistantHolidayDateFilter', function () {
    window.livewire.emit('changeDateFilter', $(this).val())
})

listenClick('.holiday-delete-btn-assistants', function (event) {
    let holidayRecordId = $(event.currentTarget).attr('data-id')
    deleteItem(route('assistants.holiday-assistant-destroy', holidayRecordId), Lang.get('messages.holiday.holiday'))
})
...
// listenClick('#assistantHolidayResetFilter', function () {
//     $('#assistantHolidayDateFilter').data('daterangepicker').setStartDate(moment().startOf('week').format('DD/MM/YYYY'))
//     $('#assistantHolidayDateFilter').data('daterangepicker').setEndDate(moment().endOf('week').format('DD/MM/YYYY'))
//     hideDropdownManually($('#assistantHolidayFilterBtn'), $('.dropdown-menu'));
// })
