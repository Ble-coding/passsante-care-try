...
document.addEventListener('turbo:load', loadAssistantData)
...
function loadAssistantDate () {
    let assistantDob = '.assistant-dob'
    let lang = $('.currentLanguage').val()
    $('.showQualification').slideUp();

    if (!$(assistantDob).length) {
        return
    }

    $(assistantDob).flatpickr({
        "locale": lang,
        maxDate: new Date(),
        disableMobile: true,
    })

    if (!$('#assistantCountryId').val()) {
        return;
    }
    $('#editassistantCountryId').val($('#assistantCountryId').val()).trigger('change')

    setTimeout(function () {
        $('#editassistantStateId').val($('#assistantStateId').val()).trigger('change')
    }, 400)

    setTimeout(function () {
        $('#editassistantCityId').val($('#assistantCityId').val()).trigger('change')
    }, 7000)
}
...
listenClick('#ResetForm', function (){
   window.location.href =  route('assistants.index')
})
...
listenClick('#saveQualification', function (e) {
    ...
    year = $('.year').val()
    let existId = $('#assistantQualificationTbl tr:last-child td:first-child').data('value')
    existId++
    if (existId) {
        id = existId
    }
...
listenSubmit('#editAssistantForm', function (e) {
    ...
    e.preventDefault()
    let assistantFormData = new FormData($(this)[0]);
    let editAssistantId = $('#editAssistantId').val();

    assistantFormData.append('qualifications', JSON.stringify(qualification))
    $.ajax({
        url: route('assistants.update', editAssistantId),
        type: 'POST',
...
listenChange('#editAssistantCountryId', function () {
    let assistantIsEdit = $('#assistantIsEdit').val();
    $.ajax({
        url: route('get-state'),
        type: 'get',
        dataType: 'json',
        ...
        success: function (data) {
            $('#editAssistantStateId').empty()
            $('#editAssistantCityId').empty()
            //
            $('#editAssistantStateId').append(
                $('<option value=""></option>').text(Lang.get('js.select_state')))
            $('#editAssistantCityId').append(
                $('<option value=""></option>').text(Lang.get('js.select_city')))
            $.each(data.data, function (i, v) {
                $('#editAssistantStateId').append($('<option></option>').attr('value', i).text(v))
            })
            if (assistantIsEdit && $('#assistantStateId').val()) {
                $('#stateId').val($('#assistantStateId').val()).trigger('change')
            }
        },
    })
})

listenChange('#editAssistantStateId', function () {
    let assistantIsEdit = $('#assistantIsEdit').val();
    $.ajax({
        url: route('get-city'),
        type: 'get',
        ...
        data: {
            state: $(this).val(),
            country: $('#editAssistantCountryId').val(),
        },
        success: function (data) {
            $('#editAssistantCityId').empty()
            $('#editAssistantCityId').append(
            ...
            if (assistantIsEdit && $('#assistantCityId').val()) {
                $('#cityId').val($('#assistantCityId').val()).trigger('change')
            }
        },
    })
})

if ($('#assistantIsEdit').val() && $('#assistantCountryId').val()) {
    $('#editAssistantCountryId').val($('#assistantCountryId').val()).trigger('change')
}
...
listenSubmit('#createAssistantForm', function () {
    let twitterUrl = $('#twitterUrl').val()
    let linkedinUrl = $('#linkedinUrl').val()
    let instagramUrl = $('#instagramUrl').val()
    let twitterExp = new RegExp(
