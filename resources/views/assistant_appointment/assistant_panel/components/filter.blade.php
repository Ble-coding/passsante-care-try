    ...
    <div class="col-md-12">
        <div class="d-flex justify-content-end">
            <div class="d-flex align-items-center">
                ...
                <span class="me-4">{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[1]))}}</span>
                <span class="badge bg-success badge-circle me-1 slot-color-dot"></span>
                <span class="me-4">{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[2]))}}</span>
                <span class="badge bg-warning badge-circle me-1 slot-color-dot"></span>
                <span class="me-4">{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[3]))}}</span>
                <span class="badge bg-danger badge-circle me-1 slot-color-dot"></span>
                <span class="me-4">{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[4]))}}</span>
            </div>
        </div>
    </div>
</div> --}}
<div class="d-flex flex-xxl-row flex-column  mt-md-0 mt-sm-3">
    ...
    <div class="d-flex justify-content-end">
       ...
       <div class="d-flex align-items-end">
        <a href="{{ route('assistants.appointments-assistant.calendar') }}" class="btn btn-icon btn-primary me-2 ms-xl-3">
        ...
        <div class="dropdown d-flex align-items-center" wire:ignore>
            <button class="btn btn btn-icon btn-primary text-white dropdown-toggle hide-arrow ps-2 pe-0"
                type="button" id="assistantPanelApptFilterBtn" data-bs-auto-close="outside" data-bs-toggle="dropdown"
                aria-expanded="false">
                <p class="text-center">
                    <i class='fas fa-filter'></i>
                </p>
            </button>
            <div class="dropdown-menu py-0" aria-labelledby="assistantPanelApptFilterBtn">
                ...
                <div class="p-5">
                    <div class="mb-5">
                        ...
                        {{ Form::select(
                            ...
                            \App\Models\AppointmentAssistant::PAYMENT_TYPE_ALL,
                            [
                                'class' => 'form-control form-control-solid form-select',
                                'data-control' => 'select2',
                                'id' => 'assistantPanelPaymentType',
                            ],
                        ) }}
                    </div>
                    <div class="mb-5">
                        <label for="filterBtn" class="form-label">{{ __('messages.appointment.status') }}:</label>
                        {{ Form::select('status', collect($filterHeads[1])->toArray(), \App\Models\AppointmentAssistant::BOOKED, [
                            'class' => 'form-control form-control-solid form-select',
                            'data-control' => 'select2',
                            'id' => 'assistantPanelAppointmentStatus',
                        ]) }}
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="reset" class="btn btn-secondary"
                            id="assistantPanelApptResetFilter">{{ __('messages.common.reset') }}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-wrap justify-content-end">
            <div class="mt-3 ms-3">
                ...
                placeholder="Pick date range" id="assistantPanelAppointmentDate" />
            </div>
            <a type="button" class="btn btn-primary mt-3 ms-3" href="{{ route('assistants.asstts-apptmt.create')}}" data-turbo="false">
