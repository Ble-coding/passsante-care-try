            ...
            {{-- <div class="tab-content" id="myTabContent"> --}}
                {{-- <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview-tab"> --}}
                    <div class="row">
                        ...
                        <div class="col-md-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant.status') }}:</label>
                            <span class="fs-4 text-gray-800">
                                <span class="badge bg-{{ getStatusBadgeColor($appointment['data']->status)}}">
                                    {{\App\Models\AppointmentAssistant::STATUS[$appointment['data']->status]}}
                                </span>
                            </span>
                        </div>

                        <div class="col-md-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant.assistant') }}:</label>
                            <span class="fs-4 text-gray-800">
                                <a class="text-decoration-none text-gray-800">
                                    {{$appointment['data']->assistant->user->full_name}}
                                </a>
                            </span>
                        </div>
                        <div class="col-md-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant_appointment.service') }}:</label>
                            <span class="fs-4 text-gray-800">{{$appointment['data']->services->name}}</span>
                        </div>
                        <div class="col-md-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant_appointment.amount') }}:</label>
                        ...
                        <div class="col-md-6 d-flex flex-column mb-5">
                            <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant_appointment.status') }}:</label>
                            <span class="fs-4 text-gray-800">
                                <span class="badge bg-{{($appointment['data']->payment_type === \App\Models\AppointmentAssistant::PAID)?'success':'danger'}}">
                                    {{($appointment['data']->payment_type === \App\Models\AppointmentAssistant::PAID)?'PAID':'PENDING'}}
                                </span>

                            </span>
                        </div>

                        @if($appointment['data']->payment_type === \App\Models\AppointmentAssistant::PAID)
                            <div class="col-md-6 d-flex flex-column">
                                <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.assistant_appointment.payment_method') }}:</label>
                                <span class="fs-4 text-gray-800">
                                            {{ !empty($appointment['data']->payment_method) ? \App\Models\AppointmentAssistant::PAYMENT_METHOD[$appointment['data']->payment_method] : __('messages.common.n/a') }}
                                        </span>
                            </div>
                        @endif
                        <div class="col-md-6 d-flex flex-column">
