                            ...
                            <div class="w-50 d-flex align-items-center">
                                ...
                                <select class="io-select2 form-select appointment-status-change appointment-status"
                                    ...
                                    <option class="booked" disabled value="{{ $book}}" {{$appointment['data']->status ==
                                                $book ? 'selected' : ''}}>{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[1]))}}
                                    </option>
                                    <option value="{{ $checkIn}}" {{$appointment['data']->status ==
                                        ...
                                        ? 'disabled'
                                        : ''}}>{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[2]))}}
                                    </option>
                                    <option value="{{ $checkOut}}" {{$appointment['data']->status ==
                                                $checkOut ? 'selected' : ''}} {{($appointment['data']->status == $cancel ||
                                        $appointment['data']->status == $book) ? 'disabled' : ''}}>{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[3]))}}
                                    </option>
                                    <option value="{{$cancel}}" {{$appointment['data']->status ==
                                                $cancel ? 'selected' : ''}} {{$appointment['data']->status == $checkIn
                                        ? 'disabled'
                                        : ''}} {{$appointment['data']->status == $checkOut ? 'disabled' : ''}}>{{__('messages.common.'.strtolower(\App\Models\AppointmentAssistant::STATUS[4]))}}
                                    </option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 d-flex flex-column mb-md-10 mb-5">
                            ...
                            <span class="fs-4 text-gray-800">
                                {{-- @if(Auth::user()->hasRole('assistant'))
                                    <a href="{{route('assistants.assistants.detail',$appointment['data']->assistant->id)}}" class="text-decoration-none">
                                        {{$appointment['data']->assistant->user->full_name}}
                                    </a>
                                @else --}}
                                    <a href="{{route('assistants.show',$appointment['data']->assistant->id)}}" class="text-decoration-none">
                                        {{$appointment['data']->assistant->user->full_name}}
                                    </a>
                                {{-- @endif --}}
                            </span>
                ...
                {{-- <div class="tab-pane fade" id="user" role="tabpanel" aria-labelledby="user-tab">
                    <div class="row">
                        <div>
                            ...
                            {{Form::hidden('assistantUrl',url('assistants'),['id'=>'indexPrescriptionAssistantUrl'])}}
