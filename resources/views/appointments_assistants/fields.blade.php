<div class="row">
    {{-- stocke un booléen indiquant si l'utilisateur connecté a le rôle de "assistant" --}}
    @role('assistant')
        {{ Form::hidden('assistantRole', getLogInUser()->hasRole('assistant'), ['id' => 'assistantRole']) }}
        {{ Form::hidden('assistant_id', getLogInUser()->assistant->id, ['id' => 'adminAppointmentAssistantId']) }}
        {{-- stocke l'identifiant du médecin associé à l'utilisateur connecté --}}
        {{ Form::hidden('status', \App\Models\AppointmentAssistant::BOOKED) }}
    @else
        <div class="col-sm-12 col-lg-6 mb-5">
            {{ Form::label('Assistant', __('messages.assistant.assistant') . ':', ['class' => 'form-label required']) }}
            {{ Form::select('assistant_id', $data['assistants'], null, ['class' => 'io-select2 form-select', 'id' => 'adminAppointmentAssistantId', 'data-control' => 'select2', 'required', 'placeholder' => __('messages.assistant.assistant')]) }}
        </div>
    ...
    <div class="col-lg-6 mb-5 col-sm-12">
        ...
        {{ Form::text('date', null, ['class' => 'form-control date', 'placeholder' => __('messages.appointment.date'), 'id' => 'appointmentAssistantDate', 'required', 'autocomplete' => 'off']) }}
    </div>
    ...
    @else
        <div class="col-lg-6 col-sm-12 mb-5">
            ...
            {{ Form::select('service_id', $data['serviceAssistants'], null, ['class' => 'io-select2 form-select', 'data-control' => 'select2', 'id' => 'appointmentAssistantServiceId', 'placeholder' => __('messages.appointment.service'), 'required']) }}
        </div>
        {{ Form::hidden('status', \App\Models\AppointmentAssistant::BOOKED) }}
        <div class="col-sm-12 col-lg-6 mb-5">
    ...
    <div class="col-lg-6 col-sm-12 mb-5">
        {{ Form::label('Service', __('messages.appointment.service') . ':', ['class' => 'form-label required']) }}
        {{ Form::select('service_id', $data['serviceAssistants'], null, ['class' => 'io-select2 form-select', 'data-control' => 'select2', 'id' => 'appointmentServiceId', 'placeholder' => __('messages.common.select_service'), 'required']) }}
    </div>
    @endrole
