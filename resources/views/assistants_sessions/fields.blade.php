@php($weekDays = App\Models\ClinicScheduleAssistant::WEEKDAY_FULL_NAME)
@php($gaps = App\Models\AssistantSession::GAPS)
@php($sessionMeetingTime = App\Models\AssistantSession::SESSION_MEETING_TIME)
@php($clinicSchedule = App\Models\ClinicScheduleAssistant::all())
<div class="row">
    <div class="col-12">
            ...
            <div class="row">
                        ...
                        <div class="pb-2 d-flex align-items-center justify-content-lg-between">
                            <div class="me-3">
                                <div class="symbol-label image image-circle image-circle image-mini">
                                    <img src="{{$doctorSession->assistant->user->profile_image}}" class="object-cover"
                                         alt=""/>
                                </div>
                            </div>
                            <span class="fs-3 fw-bolder me-3">{{$doctorSession->assistant->user->full_name}}</span>
                        </div>
                    </div>
                @endif
                ...
                @elseif(!isset($sessionWeekDays))
                    <div class="{{ isset($doctorSession) ? 'col-xl-6' : 'col-xl-4' }} col-sm-6 p-0">
                        <div class="my-4 ms-3">
                            {{ Form::label('assistant_id',__('messages.assistant.assistant').':' ,['class' => 'form-label required']) }}
                            {{ Form::select('assistant_id', $doctorsList, null,['class' => 'form-control', 'data-control'=>'select2','placeholder' => __('messages.common.select_assistant'),'required']) }}
                        </div>
                    </div>
                @endif
                <div class="{{ isset($doctorSession) ? 'col-xl-6' : 'col-xl-4' }} col-sm-6 ps-0">
                    <div class="my-4 ms-3">
                        {{ Form::label('session_gap',__('messages.assistant_session.session_gap').':' ,['class' => 'form-label required']) }}
                        {{ Form::select('session_gap', $gaps,isset($sessionWeekDays)  ? null : $gaps[array_key_first($gaps)],
                        ['class' => 'form-control','data-width' => '100%', 'data-control'=>'select2','id' => 'selGap', 'placeholder' => __('messages.assistant_session.select_session_gap'),'required']) }}
                    </div>
                </div>
                <div class="{{ isset($doctorSession) ? 'col-xl-6' : 'col-xl-4' }} col-sm-6">
                    <div class="my-4 ms-3">
                        {{ Form::label('session_meeting_time',__('messages.assistant_session.session_meeting_time').':' ,['class' => 'form-label required']) }}
                    ...
                    ['class' => 'form-control','data-width' => '100%', 'data-control'=>'select2' ,'placeholder' => __('messages.assistant_session.select_meeting_time'),'required']) }}
                    </div>
                </div>
            </div>
            @foreach(App\Models\ClinicScheduleAssistant::WEEKDAY as $day => $shortWeekDay)
                ...
                <div class="weekly-content " data-day="{{$day}}">
                    <div class="d-flex w-100 align-items-center position-relative border-bottom">
                        <div class="d-flex w-100 weekly-row my-3 ">
                            ...
                            @if(isset($sessionWeekDays))
                                @if(!$isValid)
                                    <div class="unavailable-time">{{ __('messages.assistant_session.unavailable') }}</div>
                                @endif
                            @elseif($loop->last || !$clinicScheduleDay)
                                <div class="unavailable-time">{{ __('messages.assistant_session.unavailable') }}</div>
                            @endif
                                ...
                                @if($clinicScheduleDay)
                                    ...
                                    @if(isset($sessionWeekDays) && $sessionWeekDays->count())
                                        @foreach($sessionWeekDays->where('day_of_week',$day) as $weekDaySlot)
                                            @include('assistants_sessions.slot',['slot' => $slots,'day' => $day,'weekDaySlot' => $weekDaySlot])
                                        @endforeach
                                    @else
                                        @if(!$loop->last)
                                            @if(!isset($sessionWeekDays) || $isValid)
                                                @include('assistants_sessions.slot',['slot' => $slots,'day' => $day])
                                            @endif
                                        @else
                                            ...
                                            <div class="menu-item">
                                                <div class="menu-content">
                                                        ...
                                                        @if($day != $weekDayKey && $clinicSchedule->where('day_of_week',$weekDayKey)->count())
                                                            <div
                                                                ...
                                                                <label class="form-check-label w-100 ms-0 cursor-pointer"
                                                                       for="chkCopyDay_{{$shortWeekDay}}_{{ __('messages.assistant_session.'.strtolower($weekDay)) }}">
                                                                    {{ __('messages.assistant_session.'.strtolower($weekDay)) }}
                                                                </label>
                                                    ...
                                                    <button type="button" class="btn btn-primary w-100 copy-btn"
                                                            data-copy-day="{{$day}}">
                                                        {{ __('messages.assistant_session.copy') }}</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
