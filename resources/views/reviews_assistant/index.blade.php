...
@section('content')
                ...
                <div class="row">
                    ...
                    @forelse($assistants as $assistant)
                        <div class="col-md-6 col-xxl-4 ribbon ribbon-top ribbon-vertical mt-5">
                            @if($assistant->reviews_assistant->avg('rating') != 0)
                                <div class="ribbon-label bg-primary align-items-center">
                                    <div class="star-ratings">
                                        <div class="avg-review-star-div">
                                            @php
                                                $rating = $assistant->reviews_assistant->avg('rating');
                                            @endphp
                            ...
                            <div class="card h-100">
                                <div class="card-body d-flex flex-center flex-column">
                                    <div class="image image-circle image-medium text-center">
                                        <img src="{{ $assistant->user->profile_image }}" alt="image">
                                    </div>
                                    <span class="fs-4 text-gray-800 fw-bolder text-center mt-2">{{ $assistant->user->full_name }}</span>
                                    <div class="fw-bold text-gray-400 mb-6 text-center">
                                        @foreach($assistant->specializations as $specialization)
                                            @if($loop->last)
                                                {{ $specialization->name }}
                                            @else
                                                {{ $specialization->name }},
                                    ...
                                    @forelse($assistant->reviews_assistant->where('patient_id', getLogInUser()->patient->id) as $review)
                                    ...
                                    @empty
                                        <div class="mt-auto text-center">
                                            <button class="btn btn-primary addReviewBtn" data-id="{{ $assistant->id }}"
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="card-px text-center pb-5"><h2
                                            class="fs-2x fw-bold mb-0">{{ __('messages.review.no_assistants_available_to_give_rating') }}</h2>
                                </div>
                            </div>
                        </div>
                    @endforelse
    ...
    @include('reviews_assistant.review_modal')
    @include('reviews_assistant.edit_review_modal')
@endsection
