...
class HolidayContoller extends AppBaseController
{
    ...
    public function store(CreateHolidayRequest $request): RedirectResponse
    {
        ...
        $holiday = $this->holidayRepository->store($input) ?  1:  0; //cette ligne était absente
    ...
    // public function V1store(CreateHolidayRequest $request): RedirectResponse
    // {
    //     $input = $request->all();
    //     $isnot = 0;
    //     $appointmentdate = Appointment::whereDoctorId($input['doctor_id'])->pluck('date', 'id');
    ...
    public function doctorDestroy($id): mixed
    {
        ...
        $doctorHoliday->delete(); // Notez l'utilisation de delete() plutôt que destroy($id) sur l'instance
