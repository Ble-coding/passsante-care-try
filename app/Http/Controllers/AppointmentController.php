...
class AppointmentController extends AppBaseController
{
    ...
    public function show(Appointment $appointment)
    {
        ...
        if ($user->hasRole('doctor')) {
            $doctor = Appointment::whereId($appointment->id)->whereDoctorId($user->doctor->id);
            if (! $doctor->exists()) {
                return redirect()->back();
            }
        } elseif ($user->hasRole('patient')) {
            $patient = Appointment::whereId($appointment->id)->wherePatientId($user->patient->id);
            if (! $patient->exists()) {
                return redirect()->back();
            }
        }
        ...
        if ($user->hasRole('patient')) {
            return view('patient_appointments.show', $viewData);
        } else {
            return view('appointments.show', $viewData);
        }
    }
