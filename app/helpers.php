...
if (!function_exists('getDashboardURL')) {
    ...
    function getDashboardURL()
    {
        ...
        if (Auth::user() !== null) {
            ...
            if (in_array('manage_assistant', $permissions, true)) {
            ...
            if (in_array('manage_assistant_appointments', $permissions, true)) {
                return 'admin/appointments-assistant';
            }
            ...
            // Les redirections pour l'assistant (travailleur social)

            if (in_array('manage_profils_sociaux_usagers', $permissions, true)) {
                return 'assistants/transactions';
            }
        }
...
if (!function_exists('getAssistantSessionURL')) {

    /**
     * @return string
     */
    function getAssistantSessionURL()
    {
        if (Auth::user()->hasRole('clinic_admin')) {
            return 'admin/assistant-sessions';
        } elseif (Auth::user()->hasRole('assistant')) {
            return 'assistants/assistant-sessions';
        } elseif (Auth::user()->hasRole('patient')) {
            return 'patients/assistant-sessions';
        }
...
if (!function_exists('getLoginDoctorSessionUrl')) {
    ...
    {
        return DoctorSession::toBase()->whereDoctorId(getLogInUser()->doctor->id)->exists() ?
        route('doctors.doctor.schedule.edit') : route('doctors.doctor-sessions.create');
    }
}

if (!function_exists('getLoginAssistantSessionUrl')) {

    function getLoginAssistantSessionUrl(): string
    {
        return AssistantSession::toBase()->whereAssistantId(getLogInUser()->assistant->id)->exists() ? route('assistants.schedule.assistant.edit') : route('assistants.assistant-sessions.create');
    }
}
...
if (!function_exists('getCurrencyIcon')) {
    ...
    function getCurrencyIcon()
    {
        ...
        $currencyIcon = $currency->currency_icon ?? 'FR';
...
// POUR TRAVAILLEUR SOCIAL

if (!function_exists('getAllPaymentStatusAssistant')) {

    /**
     * @return string[]
     */
    function getAllPaymentStatusAssistant()
    {
        $paymentGateway = \App\Models\AppointmentAssistant::PAYMENT_METHOD;

        $selectedPaymentGateway = PaymentGatewayAssistant::pluck('payment_gateway', 'payment_gateway_id')->toArray();
...
if (!function_exists('getPaymentGatewayAssistant')) {

    /**
     * @return string[]
     */
    function getPaymentGatewayAssistant()
    {

        $paymentGateway = \App\Models\AppointmentAssistant::PAYMENT_GATEWAY;
        $selectedPaymentGateway = PaymentGatewayAssistant::pluck('payment_gateway')->toArray();
...
// Visiteur
if (!function_exists('getVisiteurUniqueId')) {

    function getVisiteurUniqueId()
    {
        return mb_strtoupper(Patient::generateVisiteurUniqueId());
    }
}
...
if (!function_exists('isRole')) {
    ...
    // Définition de la fonction ISROLE pour la verification des menu template utilisateur
