...
class DashboardRepository
{
    ...
    public function getData(): array
    {
        ...
        $data['upcomingAppointmentCount'] = Appointment::where('date', '>',
            $todayDate)->count();
        $data['tomorrowAppointmentCount'] = Appointment::where('date', '',
        $todayDate)->count();
...
//Patiens
    ...
    // LES FONCTION CONCERNANT L'ASSISTANT
    ...
    public function getAssistantData()
    {
        ...
        $todayDate = Carbon::now()->format('Y-m-d');
        $appointments['records'] = AppointmentAssistant::with(['patient.user'])
            ->where('assistant_id', $assistantId)
            ->whereStatus(AppointmentAssistant::BOOKED)
            ->whereDate('date', Carbon::today())
            ->orderBy('date', 'ASC')
            ->paginate(5);
        $appointments['totalAppointmentCount'] = AppointmentAssistant::whereAssistantId($assistantId)->whereNotIn('status',
            [AppointmentAssistant::CANCELLED])->count();
        $appointments['todayAppointmentCount'] = AppointmentAssistant::whereAssistantId($assistantId)->where('date', '=',
            $todayDate)->whereNotIn('status', [AppointmentAssistant::CANCELLED])->count();
        $appointments['upcomingAppointmentCount'] = AppointmentAssistant::whereAssistantId($assistantId)->where('date', '>',
            $todayDate)->whereStatus(AppointmentAssistant::BOOKED)->count();
    ...
    public function assistantAllAppointment()
    {
        $assistantId = getLogInUser()->assistant->id;

        $doctorappointments = AppointmentAssistant::with(['patient.user', 'user', 'services'])
        ->where('assistant_id', $assistantId)
        ->whereYear('created_at', Carbon::now()->year)
        ->select(DB::raw('MONTH(date) as month,appointment_assistants.*'))->get();

        $transactions = TransactionAssistant::with(['user','appointment'])
         ->whereHas('appointment', function ($query) use ($assistantId) {
            $query->where('assistant_id', $assistantId);
        })
        ->whereStatus('1')
        ->select(DB::raw('MONTH(created_at) as month,transactions_assistants.*'))
        ->get();
