...
use App\Models\AssistantAppointment;
 ...
 */
class AssistantAppointmentRepository extends BaseRepository
{
    /**
    ...
    {
        return AssistantAppointment::class;
    }
    ...
    public function store($input)
    {
        try {
            ...
            $input['appointment_unique_id'] = strtoupper(AssistantAppointment::generateAppointmentUniqueId());
            ...
            $input['payment_type'] = AssistantAppointment::MANUALLY;
            $input['payment_method'] = AssistantAppointment::MANUALLY;

            $appointment = AssistantAppointment::create($input);
            ...
            $assistant = Assistant::whereId($input['assistant_id'])->with('user')->first();
            $input['assistant_name'] = $assistant->user->full_name;
            if ($assistant->user->email_notification) {
                Mail::to($assistant->user->email)->send(new AssistantAppointmentBookMail($input));
            }

            $assistantNotification = Notification::create([
                'title' => $patient->user->full_name.' '.Notification::APPOINTMENT_CREATE_ASSISTANT_MSG.' '.$input['full_time'],
                'type' => Notification::BOOKED,
    ...
    public function frontSideStore($input)
    {
        try {
            ...
            $input['to_time'] = $toTime[0];
            $input['to_time_type'] = $toTime[1];
            $input['status'] = AssistantAppointment::BOOKED;
            $input['payment_type'] = AssistantAppointment::MANUALLY;
            $appointment = AssistantAppointment::create($input);
            ...
            $assistantNotification = Notification::create([
                'title' => $patientFullName.' '.Notification::APPOINTMENT_CREATE_ASSISTANT_MSG.' '.$input['full_time'],
                'type' => Notification::BOOKED,
                'user_id' => $assistant->user->id,
            ]);
    ...
    {
        // $data['doctors'] = Assistant::with('user')->get()->where('user.status', User::ACTIVE)->pluck('user.full_name','id');
        ...
        $data['patientStatus'] = AssistantAppointment::PATIENT_STATUS;
    ...
    {
        $input = AssistantAppointment::with(['patient.user', 'patient.address', 'assistant.user', 'services'])->where('id',
            $input->id)->first();
    ...
    {
        $assistantId = getLogInUser()->assistant->id;
        /** @var Appointment $appointment */
        $appointments = AssistantAppointment::with(['patient.user', 'user'])->where('assistant_id', $assistantId)->get();
        $data = [];
        $count = 0;
    ...
    public function getPatientAppointmentsCalendar(): array
    {
        ...
        /** @var Appointment $appointment */
        $appointments = AssistantAppointment::with(['assistant.user', 'user'])->where('patient_id', $patientId)->get();
        $data = [];
        $count = 0;
        foreach ($appointments as $key => $appointment) {
            $startTime = $appointment->from_time.' '.$appointment->from_time_type;
    ...
    {
        /** @var Appointment $appointment */
        $appointments = AssistantAppointment::with(['assistant.user', 'user'])->get();
        $data = [];
        $count = 0;
        foreach ($appointments as $key => $appointment) {
            $startTime = $appointment->from_time.' '.$appointment->from_time_type;
    ...
    public function showAppointment($input): array
    {
        ...
        $data['data'] = AssistantAppointment::with(['patient.user', 'assistant.user', 'services'])->findOrFail($input['id']);
    ...
    {
        $data['data'] = AssistantAppointment::with(['patient.user', 'assistant.user', 'services'])->findOrFail($appointment->id);
    ...
    public function createSession($input)
    {
        ...
        $assistantName = Assistant::with('user')->whereId($input['assistant_id'])->first();
        setStripeApiKey();
        ...
        $session = Session::create([
            ...
            'line_items' => [
                [
                    ...
                    'quantity' => 1,
                    'description' => 'Payment for booking appointment with assistant :
                     '.$assistantName->user->full_name.' at '.Carbon::parse($input->date)->format('d/m/Y').' '.$input->from_time.' '.$input->from_time_type.' to '.$input->to_time.' '.$input->to_time_type,
                ],
            ],
