...
use App\Mail\AssistantAppointmentBookMail;
 ...
 */
class AppointmentAssistantRepository extends BaseRepository
{
    /**
    ...
    {
        return AppointmentAssistant::class;
    }
    ...
    public function store($input)
    {
        try {
            ...
            $input['appointment_unique_id'] = strtoupper(AppointmentAssistant::generateAppointmentUniqueId());
            ...
            $input['payment_type'] = AppointmentAssistant::MANUALLY;
            $input['payment_method'] = AppointmentAssistant::MANUALLY;

            $appointment = AppointmentAssistant::create($input);
            ...
            $service = ServiceAssistant::whereId($input['service_id'])->first();
            ...
            if (! getLogInUser()->hasRole('patient')) {
                $patientNotification = NotificationAssistant::create([
                    'title' => NotificationAssistant::APPOINTMENT_CREATE_PATIENT_MSG.' '.$input['full_time'],
                    'type' => NotificationAssistant::BOOKED,
                    'user_id' => $patient->user->id,
                ]);
            }

            $doctor = Assistant::whereId($input['assistant_id'])->with('user')->first();
            $input['assistant_name'] = $doctor->user->full_name;
            if ($doctor->user->email_notification) {
                Mail::to($doctor->user->email)->send(new AssistantAppointmentBookMail($input));
            }

            $doctorNotification = NotificationAssistant::create([
                'title' => $patient->user->full_name.' '.NotificationAssistant::APPOINTMENT_CREATE_ASSISTANT_MSG.' '.$input['full_time'],
                'type' => NotificationAssistant::BOOKED,
    ...
    public function frontSideStore($input)
    {
        try {
            ...
            $input['to_time'] = $toTime[0];
            $input['to_time_type'] = $toTime[1];
            $input['status'] = AppointmentAssistant::BOOKED;
            $input['payment_type'] = AppointmentAssistant::MANUALLY;
            $appointment = AppointmentAssistant::create($input);

            Mail::to($input['email'])->send(new AssistantAppointmentBookMail($input));
            ...
            $doctorNotification = NotificationAssistant::create([
                'title' => $patientFullName.' '.NotificationAssistant::APPOINTMENT_CREATE_ASSISTANT_MSG.' '.$input['full_time'],
                'type' => NotificationAssistant::BOOKED,
                'user_id' => $doctor->user->id,
            ]);
    ...
    {
        $data['assistants'] = Assistant::with('user')->get()->where('user.status', User::ACTIVE)->pluck('user.full_name','id');
        $data['assistants'] = Assistant::with('user')->get()->where('user.status', User::ACTIVE)->pluck('user.full_name','id');
        $data['patients'] = Patient::with('user')->get()->pluck('user.full_name', 'id');
        $data['patientStatus'] = AppointmentAssistant::PATIENT_STATUS;
        $data['serviceAssistants'] = ServiceAssistant::whereStatus(ServiceAssistant::ACTIVE)->pluck('name', 'id');
    ...
    {
        $input = AppointmentAssistant::with(['patient.user', 'patient.address', 'assistant.user', 'services'])->where('id',
            $input->id)->first();
        ...
        $data['assistant'] = $input->assistant->user->full_name;
    ...
    {
        $doctorId = getLogInUser()->assistant->id;
        /** @var Appointment $appointment */
        $appointments = AppointmentAssistant::with(['patient.user', 'user'])->where('assistant_id', $doctorId)->get();
        $data = [];
        $count = 0;
    ...
    public function getPatientAppointmentsCalendar(): array
    {
        ...
        /** @var AppointmentAssistant $appointment */
        $appointments = AppointmentAssistant::with(['assistant.user', 'user'])->where('patient_id', $patientId)->get();
        $data = [];
        $count = 0;
        foreach ($appointments as $key => $appointment) {
            $startTime = $appointment->from_time.' '.$appointment->from_time_type;
            ...
            $data[$key]['id'] = $appointment->id;
            $data[$key]['title'] = $startTime.'-'.$endTime;
            $data[$key]['assistantName'] = $appointment->assistant->user->full_name;
            $data[$key]['start'] = $start->toDateTimeString();
            $data[$key]['description'] = $appointment->description;
            $data[$key]['status'] = $appointment->status;
            $data[$key]['amount'] = $appointment->payable_amount;
    ...
    {
        /** @var Appointment $appointment */
        $appointments = AppointmentAssistant::with(['assistant.user', 'user'])->get();
        $data = [];
        $count = 0;
        foreach ($appointments as $key => $appointment) {
            $startTime = $appointment->from_time.' '.$appointment->from_time_type;
    ...
    public function showAppointment($input): array
    {
        ...
        $data['data'] = AppointmentAssistant::with(['patient.user', 'assistant.user', 'services'])->findOrFail($input['id']);

        $data['transactionStatus'] = TransactionAssistant::whereAppointmentId($data['data']->appointment_unique_id)->exists();

        return $data;
    }

    public function showAssistantAppointment($appointment): array
    {
        $data['data'] = AppointmentAssistant::with(['patient.user', 'assistant.user', 'services'])->findOrFail($appointment->id);
    ...
    public function createSession($input)
    {
        ...
        $doctorName = Assistant::with('user')->whereId($input['assistant_id'])->first();
        setStripeApiKey();
        ...
        $session = Session::create([
            ...
            'line_items' => [
                [
                    ...
                    'quantity' => 1,
                    'description' => 'Payment for booking appointment with travailleur social :
